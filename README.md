...
功能闭环且没有相反证据，才归入“今日完成工作”。功能不完整时归入“今日未完成工作”。未提交、仅本地提交、是否已在上游可见等信息只作为内部判定证据，不进入默认日报正文，也不会单独把一个完整功能降为未完成。
...
定稿前读取 references/humanizer-zh.md。测试、构建、命令执行结果和 Git 状态只用于内部判断；最终正文保留业务事实、数字和可观察结果，再做表达调整：

- 用“完成了什么、解决了什么问题、当前呈现什么结果”描述工作
- 不把测试通过、构建通过、命令执行成功写成工作成果
- 不写未提交、未推送、工作区状态或上游可见状态，除非它们直接造成无法使用、评审或发布
- 少用空泛的意义判断和宣传式结论
